...
- Emit each viva question as a SEPARATE JSON object on its OWN line (JSON Lines). One object, a newline, the next object.
...
- Each line must be exactly one JSON object with these keys:
  {{"question": "...", "expected_answer": "...", "follow_ups": ["..."], "difficulty": "easy|medium|hard", "grounded_chunks": [0]}}
Begin now — one viva JSON object per line, nothing else."""

_STREAM_TASK = _TASK.replace(
    "Return a single JSON object matching the VivaSet schema.", _STREAM_FOOTER
)

VIVA_STREAM_PROMPT = ChatPromptTemplate.from_messages([
