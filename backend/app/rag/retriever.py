...
hand-rolled RAG. It does NOT replace :mod:`app.services.rag_service` —
...
def _neutralise_boundaries(text: str) -> str:
    """If a chunk happens to contain literal ``<NOTES>`` / ``</NOTES>``
    boundary markers (e.g. an attacker trying to escape the wrapper),
    replace the angle brackets so they can't close our wrapper early.
    """
...
def docs_to_numbered_context(docs: list[Document], cap_chars: int = 12000) -> str:
    ...
    Wrapped in ``<NOTES>…</NOTES>`` so the model can distinguish quoted
    user data from system instructions; literal boundary markers in chunks
    are neutralised before insertion.
    """
    out: list[str] = ["<NOTES>"]
    total = len("<NOTES>") + len("</NOTES>") + 4
    for d in docs:
        idx = d.metadata.get("chunk_idx")
        clean = " ".join(_neutralise_boundaries(d.page_content).split())
        line = f"[#{idx}] {clean}"
        ...
        total += len(line) + 2
    out.append("</NOTES>")
    if len(out) == 2:  # only opener + closer, no chunks
        return "<NOTES>\n(no notes available)\n</NOTES>"
    return "\n\n".join(out)
...
def multi_doc_numbered_context(
    ...
    Wrapped in ``<NOTES>…</NOTES>`` so the model lexically distinguishes
    quoted user data from system instructions. Each chunk is prefixed with
    ``[<doc_id>#<chunk_idx>]`` so the LLM can cite unambiguously across
    documents. Chunks are interleaved (round-robin across docs) so a budget
    cap doesn't starve later docs. Literal boundary markers inside chunks
    are neutralised so they can't close the wrapper early.
    """
    ...
    while True:
        ...
        for i, docs in enumerate(per_doc):
            ...
            text = " ".join(_neutralise_boundaries(d.page_content).split())
