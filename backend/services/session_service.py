...
from backend.utils.vad import UtteranceDetector, UtteranceStats, VadEvent
...
def _split_first_sentence(buf: str, force: bool = False) -> tuple[str, str]:
...
class Session:
    ...
    async def _answer_and_speak(self, question: str) -> None:
        ...
        async def flush_sentences(force: bool = False) -> None:
            nonlocal pending
            while True:
                sentence, remainder = _split_first_sentence(pending, force=force)
                if not sentence:
                    return
                pending = remainder
                try:
