...
export default function VivaView({ docId, action }) {
  ...
  const onGenerate = useCallback(async () => {
    ...
    setBusy(true)
    try {
      const r = await generateViva(docId, { n, force: set != null })
      setSet(r); setIdx(0); setRevealed(new Set()); setOpenFollowUps(new Set())
      toast.success('Viva ready', `${r.questions.length} questions`)
    } catch (err) {
  ...
  const performRegen = useCallback(async () => {
    setBusy(true)
    try {
      try { await deleteViva(docId) } catch {}
      const r = await generateViva(docId, { n, force: true })
      setSet(r); setIdx(0); setRevealed(new Set()); setOpenFollowUps(new Set())
